...
public class Stage1SecondaryB extends Brain {
  //---PARAMETERS---//
  private static final double HEADINGPRECISION = 0.001;
  private static final double ANGLEPRECISION = 0.1;

  private static final int LUIGI = 0x1EADDA;
  private static final int WARIO = 0x5EC0;
  ...
  public void activate() {
    ...
    for (IRadarResult o: detectRadar())
      if (isSameDirection(o.getObjectDirection(),Parameters.NORTH)) whoAmI=WARIO;
    if (whoAmI == LUIGI){
    ...
    //INIT
    state=(whoAmI==WARIO)?TURNLEFTTASK:SINK;
    isMoving=false;
    oldAngle=getHeading();
  }
  public void step() {
    //ODOMETRY CODE
    if (isMoving && whoAmI == LUIGI){
    ...
    //DEBUG MESSAGE
    if (whoAmI == LUIGI) {
      sendLogMessage("#LUIGI *thinks* he is rolling at position ("+(int)myX+", "+(int)myY+").");
    }
